...
function RightSideWindow({ closeWindow }) {
  ...
  const [squareFtValue, setSquareFtValue] = useState(1);
  ...
  const handleSearch = () => {
    navigate('/search',  {
      state: {
          blocks: blocks,
          squareFtValue: squareFtValue,
      },
  });
  }

  const squareFtValueHandler = (value) => {
    setSquareFtValue(value);
    console.log(squareFtValue);
  }
  ...
  return (
      ...
      <div style={{
        ...
        onChange={(event) => squareFtValueHandler(event.target.value)}
        name="preference">
